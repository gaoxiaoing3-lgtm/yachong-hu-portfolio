#!/usr/bin/env node
import fs from 'node:fs/promises'
import path from 'node:path'

const root = process.cwd()
const bridgeMemoryDir = process.env.WUDI_MEMORY_DIR || '/Users/jingqi/Documents/ask/weixin-codex-bridge/memory'
const outPath = path.join(root, 'public', 'wudi-memory.json')

async function read(file) {
  try { return await fs.readFile(path.join(bridgeMemoryDir, file), 'utf8') } catch { return '' }
}

function has(text, pattern) { return pattern.test(text) }

const memory = await read('MEMORY.md')
const user = await read('USER.md')
const all = `${user}\n${memory}`

const reimbursementDone = has(all, /报销表填完|全部编辑完成|142 个初步文件已上传|提交前复核/)
const oplus = has(all, /OPLUS|渲染引擎|在线包装|虚拟演播室/)
const panel = has(all, /角色面板|\/wudi|无敌心法.*MVP/)

const data = {
  updatedAt: new Date().toISOString(),
  realm: reimbursementDone ? '破局 · 五星' : '训练中',
  mainline: '先清债，再造器',
  stats: [
    { key: '护阳', name: '生命力', value: 62, stars: '★★★☆☆', note: '精气神 / 欲望管理' },
    { key: '炼骨', name: '耐力', value: reimbursementDone ? 86 : 72, stars: reimbursementDone ? '★★★★☆' : '★★★☆☆', note: '枯燥任务持续推进' },
    { key: '铸魂', name: '志气', value: 68, stars: '★★★☆☆', note: '责任 / 荣誉 / 主心骨' },
    { key: '开眼', name: '洞察', value: 74, stars: '★★★★☆', note: '看清规则与结构' },
    { key: '定心', name: '心境', value: 80, stars: '★★★★☆', note: '收束精力，不硬耗' },
    { key: '破局', name: '行动', value: reimbursementDone ? 98 : 82, stars: reimbursementDone ? '★★★★★' : '★★★★☆', note: '把混乱拆成流程' },
    { key: '藏器', name: '实力', value: panel ? 82 : 76, stars: '★★★★☆', note: '网站 / GitHub / 引擎' },
    { key: '择道', name: '命途', value: oplus ? 84 : 80, stars: '★★★★☆', note: '虚拟制作 + AIGC' },
    { key: '守门', name: '边界', value: reimbursementDone ? 90 : 80, stars: '★★★★☆', note: '财务防错 / 风险防守' }
  ],
  tasks: [
    { title: '报销提交前复核', type: '破局 + 守门', state: reimbursementDone ? '提交前复核' : '进行中', stars: reimbursementDone ? '★★★★★' : '★★★★☆' },
    { title: 'OPLUS 渲染引擎', type: '择道 + 藏器', state: oplus ? '今日主线' : '规划中', stars: '★★★★☆' },
    { title: '无敌心法角色面板', type: '藏器 + 破局', state: panel ? 'MVP 已落地' : 'MVP', stars: '★★★★☆' }
  ],
  breakthroughs: [
    reimbursementDone ? '从抗拒半年报销，到完成表格、142 个文件上传与提交前复核。' : '正在把报销从混乱拆成流程。',
    '从“技术负责人”校准为“虚拟制作与 AI 数字内容解决方案实践者”。',
    panel ? '无敌心法角色面板已落地到个人网站 /wudi。' : '把微信对话、提醒、记忆系统接成本地成长反馈闭环。'
  ]
}

await fs.mkdir(path.dirname(outPath), { recursive: true })
await fs.writeFile(outPath, JSON.stringify(data, null, 2), 'utf8')
console.log(`[sync-wudi-memory] wrote ${outPath}`)
